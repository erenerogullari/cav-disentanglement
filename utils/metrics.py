...
def get_accuracy(y_hat, y, se=False):
    ...
    # Compute accuracy
    accuracy = (
        (y_pred == y).float().mean().item()
    )  # Count samples where all classes match
...
def get_accuracy_mc(y_hat, y, se=False):
    if y.dim() == 2:
        accuracy = ((y_hat.sigmoid() > 0.5).long() == y).float().mean().item()
    else:
...
def get_f1_mc(y_hat, y):
    pred = (y_hat.sigmoid() > 0.5).long() if y.dim() == 2 else y_hat.argmax(dim=1)
...
def compute_auc_performance(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        activations_normalized = torch.nn.functional.normalize(
            activations, dim=1, eps=eps
        )
...
def get_uniqueness(cavs: torch.Tensor):
    ...
    uniqueness = (
        1
        - np.average(np.abs(cos_sim_matrix), axis=0)
        + np.abs(np.diag(cos_sim_matrix)) / n
    )
...
def get_auconf(
    y_hat: torch.Tensor, y: torch.Tensor, *, n_thresholds: int = 101
) -> np.ndarray:
    """
    ...
    thresholds = torch.linspace(
        0.0, 1.0, steps=n_thresholds, device=y_hat.device, dtype=y_hat.dtype
    )
    ...
    _trapz = getattr(np, "trapezoid", np.trapezoid)  # np.trapz removed in NumPy 2.0
    cm_integral = _trapz(cms, thr_np, axis=0)  # area under each entry‑curve
