...
"""
from models.src_diffae.model.unet_autoenc import AutoencReturn
...
from models.src_diffae.model.nn import mean_flat
...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        if self.loss_type in [
            ...
            with th.autocast(device_type='cpu', enabled=self.conf.fp16):
    ...
    def ddim_sample_loop(
        ...
        """
        final = None
        for sample in tqdm(self.ddim_sample_loop_progressive(
                model,
                shape,
                noise=noise,
                clip_denoised=clip_denoised,
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = th.from_numpy(arr).to(th.float32).to(device=timesteps.device)[timesteps]
