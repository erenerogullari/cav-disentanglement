...
def evaluate_by_subset_attacked(config, cav=None, return_cm=False):
    ...
    for split in [
        ...
        if return_cm:
            cms_all[split] = {
                "attacked": metrics_attacked[1],
                "ch": metrics[1],
                "clean": metrics_clean[1],
            }
            metrics, metrics_attacked, metrics_clean = metrics[0], metrics_attacked[0], metrics_clean[0]

        metrics_all_split = {**metrics, **metrics_attacked, **metrics_clean}
        metrics_all = {**metrics_all, **metrics_all_split}
        if config.get('wandb_api_key', None):
            wandb.log(metrics_all_split)
    if return_cm:
