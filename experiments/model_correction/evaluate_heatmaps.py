...
def evaluate_concept_heatmaps(
    ...
    for cav_name, locs in cav_localizations.items():
        for cname in ["timestamp", "box"]:
            ...
            results_quant[f"concept_rel_{cname}_{cav_name}_sem"] = concept_rel_np.std() / np.sqrt(
                len(concept_rel_np)
            )
    ...
    plot_concept_relevance_std(data_plot_std, vmax, results_dir / "concept_relevance_std")
