...
from experiments.model_correction.evaluate_by_subset_attacked import (
    evaluate_by_subset_attacked,
)
from experiments.model_correction.utils import (
    load_base_model,
    compose_clarc_config,
    build_clarc_kwargs,
...
def get_activations_ds(model, dataset, config, device, split):
    ...
    dl_split = DataLoader(
        dataset_split, batch_size=config.train.batch_size, shuffle=False
    )
    ...
    cav_mode = config.cav.cav_mode
    activations_split = None
    for x, y in dl_split:
        _ = model(x.to(device))
        if cav_mode == "full":
            acts_batch = activations.clone().detach().cpu()
        elif cav_mode == "max":
            acts_batch = (
                activations.clone()
                .detach()
            ...
            acts_batch = (
                activations.clone().detach().cpu().flatten(start_dim=2).mean(dim=2)
            )
        else:
            raise ValueError(
        ...
        activations_split = (
            acts_batch
            if activations_split is None
            else torch.cat([activations_split, acts_batch])
        )
...
def evaluate_model_correction(
    cfg: DictConfig, cav_model: nn.Module, base_model: nn.Module
) -> None:
    ...
    model_baseline = build_model(
        cfg, device, dataset=dataset, cav=base_cavs[artifact_id, :], clarc=True
    )
    model_orth = build_model(
        cfg, device, dataset=dataset, cav=orth_cavs[artifact_id, :], clarc=True
    )

    log.info("Extracting activations on test set.")
    activations_vanilla, _ = get_activations_ds(
        model_vanilla, dataset, cfg, device, split="test"
    )
    activations_clarc, metadata_concepts = get_activations_ds(
        model_baseline, dataset, cfg, device, split="test"
    )
    activations_clarc_orth, metadata_concepts = get_activations_ds(
        model_orth, dataset, cfg, device, split="test"
    )
    ...
    for c, sims in similarities_baseline.items():
        for v in sims:
            data_similarities.append(
                {"concept": c, "v": v.item(), "model": "Baseline CAV"}
            )
    for c, sims in similarities_orth.items():
        for v in sims:
            data_similarities.append(
                {"concept": c, "v": v.item(), "model": "Orthogonal CAV"}
            )
    ...
    metrics_vanilla, cm_vanilla = evaluate_by_subset_attacked(
        cfg, model_vanilla, dataset, return_cm=True
    )
    metrics_baseline, cm_baseline = evaluate_by_subset_attacked(
        cfg, model_baseline, dataset, return_cm=True
    )
    metrics_orth, cm_orth = evaluate_by_subset_attacked(
        cfg, model_orth, dataset, return_cm=True
    )
    ...
    df_filtered["Metric Type"] = df_filtered["Metric"].str.replace(
        r"_(1|ch|attacked|clean)", "", regex=True
    )
