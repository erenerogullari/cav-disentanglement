...
def _instantiate_dataset(dataset_name: str, **kwargs):
    dataset_fn = get_dataset(dataset_name)
    signature = inspect.signature(dataset_fn)
    if not any(
...
def compute_metrics(
    model_outs, y_true, classes=None, prefix="", suffix="", return_cm=False
):
    ...
    if classes is not None:
        results_auc = {
            f"{prefix}AUC_{classes[class_id]}{suffix}": get_auc_label(
                y_true, model_outs, class_id
            )
        ...
        if y_true.ndim == 1:
            ...
            ## multi-label
            model_preds = (torch.sigmoid(model_outs) > 0.5).type(torch.uint8)

        results_fpr = {
            f"{prefix}fpr_{classes[class_id]}{suffix}": get_fpr_label(
                y_true, model_preds, class_id
            )
            for class_id in range(model_preds.shape[1])
            if y_true[:, class_id].sum() > 0
        }

        results_fnr = {
            f"{prefix}fnr_{classes[class_id]}{suffix}": get_fnr_label(
                y_true, model_preds, class_id
            )
            for class_id in range(model_preds.shape[1])
            if y_true[:, class_id].sum() > 0
        ...
        results_acc = {
            f"{prefix}acc_{classes[class_id]}{suffix}": (
                y_true[:, class_id] == model_preds[:, class_id]
            )
            .numpy()
            .mean()
            for class_id in range(model_preds.shape[1])
...
def evaluate_by_subset_attacked(
    config: DictConfig, model: nn.Module, dataset: Dataset, return_cm=False
):
    ...
    dataset_clean = _instantiate_dataset(
        dataset_name,
    ...
    dataset_attacked = _instantiate_dataset(
        dataset_name,
        data_paths=data_paths,
        normalize_data=True,
        p_artifact=1.0,
    ...
    for split in [
        ...
        dl_attacked = DataLoader(
            dataset_attacked_split, batch_size=batch_size, shuffle=False
        )
        model_outs_attacked, y_true_attacked = compute_model_scores(
            model, dl_attacked, device
        )
