...
class CelebAHQWithEncodings(Dataset):
    ...
    def get_encs(self):
        ...
        self.label_to_id = {k: i for i, k in enumerate(id_to_label)}    # type: ignore
    ...
    def __getitem__(self, idx):
        ...
        img = read_image(str(self.paths[img_idx])) / 255    # type: ignore
        enc = self.xs[idx]
        label = self.ys[idx]
    ...
    def get_subset_by_idxs(self, idxs):
        ...
        """
        subset = CelebAHQWithEncodings(self.path_data, self.path_encodings, len(idxs), name=self.name)  # type: ignore
        
        # Update paths, xs, ys
        subset.paths = [self.paths[self.map_to_img_idx(i).item()] for i in idxs]    # type: ignore
